...
- **BookShare** · Share a book between devices · **planned** · ms:Enrichment · deps:[EPubAlign] · eff:M — One `.skriftbook` file (ZIP; ZIPFoundation is already dep #2) carries a book's audio + ePub + whole‑book transcript + alignment + chapters, and its reading layer **only** when the destination is your own device (Tuur 2026‑07‑30: one format, a switch). AirDrop / Save to Files / another Skrift — three entry points, one importer. Covers what per‑book CloudKit sync structurally can't: another person, no iCloud‑storage cost, and carrying just the ~15 MB of reading work instead of the audio. Designed + mocked 2026‑07‑30 (`mocks/book-sharing.html`), **sign‑off owed, not built**. Board = `backlog.md` 📦.
