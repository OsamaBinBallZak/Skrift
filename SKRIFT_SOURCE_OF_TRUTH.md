...
- **BookShare** · Share a book with someone · **planned** · ms:Enrichment · deps:[EPubAlign] · eff:M — One `.skriftbook` file (ZIP; ZIPFoundation is already dep #2) hands a book's audio + ePub + whole‑book transcript + alignment + chapters to **another person** running Skrift, over AirDrop / Files / Messages — one importer for all three. **Scope cut by Tuur 2026‑07‑30** ("all that is done over cloud sync"): not for your own devices, since `AudiobookCloudSync` already carries audio, cover, position, bookmarks, transcript **and** alignment sidecars between them — so the reading layer never travels at all, an absolute rule rather than a switch. What remains is the one thing sync structurally can't do: reach someone else. Audio optional (proposed default off) — the ~15 MB text‑only bundle saves them 28h of ANE time and redistributes nothing. Design gotcha: Dev + Prod both *exporting* one UTI makes document routing undefined, so the type must be per‑config like the bundle ids. Designed + mocked 2026‑07‑30 (`mocks/book-sharing.html`), **sign‑off owed, not built**. Board = `backlog.md` 📦.
