...
/**
 * Hook for safe theme management
 */
export function useThemeSafe() {
  ...
  // Get initial theme from Electron API
  useEffect(() => {
    if (isAvailable && api) {
      api.theme.getSystemTheme().then(setSystemTheme).catch(console.error);
    }
  }, [isAvailable, api]);
  ...
  const changeTheme = useCallback(async (newTheme: 'light' | 'dark' | 'system') => {
    ...
    if (isAvailable && api) {
      try {
        ...
        console.error('Failed to change theme via Electron API:', error);
      }
    }
  }, [isAvailable, api]);
